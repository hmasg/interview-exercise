...
# MAGIC       <ol>
# MAGIC         <li><b>Daily top-5 NFT tokens with the highest amount</b></li>
...
from pyspark.sql.functions import count, avg, col, max, row_number, to_date
...
# destination folder
DailyTop5NFT_output_folder = "/FileStore/heni/result/DailyTop5NFT/"
...
# MAGIC | `Output`    | `/FileStore/heni/result/DailyTop5NFT/DailyTop5NFT.parquet` |
...
# MAGIC %md-sandbox
# MAGIC ##### ![Spark Logo Tiny](https://files.training.databricks.com/images/105/logo_spark_tiny.png) Development: Find the highest amount for each token by date

# COMMAND ----------

df_DailyTop5NFT_stage01= df_no_duplicates.select("date","token_id","amount") \
                      .groupBy("date","token_id").agg(max("amount")) \
                      .sort(col("date"),col("max(amount)").desc())
...
windowDay = Window.partitionBy("date").orderBy(col("max(amount)").desc())
df_DailyTop5NFT_stage02 = df_DailyTop5NFT_stage01.withColumn("row",row_number().over(windowDay))
df_DailyTop5NFT = df_DailyTop5NFT_stage02.select("date","token_id", col("max(amount)").alias("amount")).filter(col("row") <= 5)
